...
c******************************************************************************
      ...
      parameter(mx=301)
      real xbase(mx),ybase(mx),cptarget(mx),cp(mx),dOdV(22),dstepnm_1
      real ybase2(mx)
...
c..Compute the gradient vector and its magnitude
      real xbase(mx),ybase(mx),cptarget(mx),cp(mx),ypert(mx),dOdV(22)

      nle = np/2 + 1
      npp = np+1
      dOdVmag = 0.
      do ndv = 1,11  !..perturb the lower surface with bump functions
        nfun = ndv
      ...
      do ndv = 12,22   !..perturb the upper surface with bump functions
        nfun = ndv-11
...
c------------------------------------------------------------------------
      ...
      enddo
      do n=ns,ne
        ypert(n) = ypert(n) + delta*BUMP_Poly(nfun,xbase(n))
...
c..New Airfoil with Bumped
      ...
      real xbase(mx),ybase(mx),dOdV(22),dstep
      integer nle, np
      nle = np/2 + 1
      
      do i=1,nle
      do ndv = 1,11
      ybase(i)=ybase(i)+dstep*dOdV(ndv)*BUMP_Poly(ndv,xbase(i))
      enddo
      enddo

      do i=nle+1,np+1
      do ndv = 12,22
      ybase(i)=ybase(i)+dstep*dOdV(ndv)*BUMP_Poly(ndv-11,xbase(i))
      enddo
      enddo
